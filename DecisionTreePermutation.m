function [accuracy,DecValues]=DecisionTreePermutation(Cfg)
%Permutation for Decision Tree (classification)
        ...
        switch strSelectFeature
            ...
            case 'F-Score'
                ...
                for j=1:length(SelectFeatureNum)
                    ...
                    model = fitctree(Frank, train_label);
                    [predict_label,dec_values] =predict(model,FC_tes);
                    acc= length(find(predict_label== test_label))/length(test_label)*100;
                    TemAcc(j,i)=acc(1);
