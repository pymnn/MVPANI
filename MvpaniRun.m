function [accuracy, Predict_test_Label,SpecificitySensitivity,weightall,SelectFeatureIndex,Specif_Sensit_acc_fold,MaxDim]=MvpaniRun(Cfg)
...
strClassAlgorithm=Cfg.ClassAlgorithmStr;
...
if ~strcmp(strFeatureReduce,'PCA (Principal Component Analysis)')
    ...
    for i=1:max(Fold)    
        ...
        if strcmp(FishZStr,'Cloumn Normalization')
...
%     model.sv_coef'
%     weight'
%     pre=sign(train_data*weight-model.rho)';
    %%
    switch strSelectFeature
        ...
        case 'Weight'
            ...
            for j=1:length(SelectFeatureNum)
                ...
                Specif_Sensit_acc_fold(i,1,j)=length(find(predict_label(find(predict_label==test_label))==ReLabel(1)))/length(find(test_label==ReLabel(1)))*100;
                Specif_Sensit_acc_fold(i,2,j)=length(find(predict_label(find(predict_label==test_label))==ReLabel(2)))/length(find(test_label==ReLabel(2)))*100;
                Specif_Sensit_acc_fold(i,3,j)=length(find(predict_label==test_label))/length(test_label)*100;
                accuracy(i,j)=acc(1);
