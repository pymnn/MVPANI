function [rr,pp]=RandomForestRegressionPermutation(Cfg)
...
parfor iper=1:PermutationNum
    ...
    for i=1:max(Fold)
        ...
        switch strSelectFeature
            case 'None'%
                ...
                TempPredictLabel(find(Fold==i),1)=predict_label;
            ...
            case 'Lasso'
                ...
                TempPredictLabel(find(Fold==i))=predict_label;
                Temptestyy(find(Fold==i))=test_label;
            case 'F-Score'
                ...
                for jNum=1:length(SelectFeatureNum)
                    ...
                    TempPredictLabel(find(Fold==i),jNum)=predict_label;
    ...
    for k=1:size(TempPredictLabel,2)
        [r,p]=corrcoef(Temptestyy(:,k),TempPredictLabel(:,k));
        rtem(k)=r(1,2);
        ptem(k)=p(1,2);
    end
    rr(iper,:)=rtem;
